...
/// Confirms whether one issue report reached the local progress database.
#[derive(Debug, Serialize, RustSignal)]
pub struct IssueReportStatus {
    pub report_id: String,
    pub success: bool,
    pub message: String,
}

/// Request counts for active local tutor gloss corrections and corrections
/// that no longer change the card generated by the current core data.
#[derive(Debug, Deserialize, DartSignal)]
pub struct GetTutorGlossOverrideStats {}

/// Recheck local tutor gloss corrections and tombstone exact no-ops so their
/// removal converges through normal progress sync.
#[derive(Debug, Deserialize, DartSignal)]
pub struct OptimizeTutorGlossOverrides {}

/// Current local correction counts. `removed` is non-zero only in the response
/// to an explicit optimization request.
#[derive(Debug, Serialize, RustSignal)]
pub struct TutorGlossOverrideStats {
    pub total: i64,
    pub redundant: i64,
    pub removed: i64,
    pub error: String,
